...
The blog section has been redesigned to take advantage of [Jekyll's tags](http://jekyllrb.com/docs/frontmatter/). Now, it's possible to filter posts by tags and see related posts.

To get started, I came across [this blog](http://www.jokecamp.com/blog/listing-jekyll-posts-by-tag/), which shows how to list all the site tags and display all the posts by tag. I expanded upon this by doing the following:
...
The `hidden` class is added to all the lists to start and is removed for the appropriate list of posts when `showContainer(tag)` is called (see the JavaScript code above). The `hidden` class is provided by [Bootstrap](http://getbootstrap.com), but you can manually add it into your own CSS file:
...
And that's pretty much it! To see it used in context, feel free to take a look at this website's source code on [GitHub](https://github.com/estherjk/estherjk.github.io).
